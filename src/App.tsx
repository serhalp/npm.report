...
export default function App() {
  ...
  const selectedKinds = (Object.keys(selected) as ReportKind[]).filter(
    (k) => selected[k],
  )
  ...
  async function handleRun() {
    ...
    log(
      all
        ? '→ scope: ALL org packages (-A)'
        : `→ scope: last ${months} months`,
    )
    try {
  ...
  async function handleShare() {
    ...
    try {
      const scopeLabel =
        result.recent?.summary.scopeLabel ??
        (all ? 'all org packages' : `last ${months} months`)
  ...
  async function handleRunUserPublishes() {
    ...
    const extra =
      upUseCache && result?.recent ? result.recent.rows.map((r) => r.pkg) : []
    setUpRunning(true)
    setUpResult(null)
    termRef.current?.clear()
    ...
    try {
      ...
      const rep = await runUserPublishes(
        user,
        upMonths,
        jobs,
        extra,
  ...
  const hasReports = !!(
    result &&
    (result.recent || result.manual || result.external)
  )
  ...
  return (
    <div className="app">
      <header className="masthead">
        ...
        <p>
          Point this at any npm organizations to track trusted-publishing /
          provenance rollout, find packages published manually rather than via
          CI, and surface maintainers who can publish but aren&rsquo;t org
          members. Everything runs in your browser against the public npm
      ...
      <div className="layout">
        ...
        <section className="panel">
          ...
          <div className="panel__body">
            <div className="field">
              ...
              />
              <p className="desc">
                One or more npm org slugs. The registry caps org listings at 250
            ...
            <div className="field">
              ...
              <div className="checks">
                {REPORT_META.map((r) => (
                  <label
                    key={r.kind}
                    className={`check${selected[r.kind] ? ' active' : ''}`}
                  >
                    <input
                      type="checkbox"
                      checked={selected[r.kind]}
                      onChange={(e) =>
                        setSelected((s) => ({ ...s, [r.kind]: e.target.checked }))
                      }
                    />
                    <span>
            ...
            <div className="field">
              ...
              />
              <p className="desc">
                Publishers to treat as automation. Note: npm cannot distinguish a
                human from that human&rsquo;s CI token, so &ldquo;manual&rdquo; is
                a proxy, not proof.
              </p>
            </div>
            ...
            {selected.external && (
              <div className="field">
                ...
                <p className="desc" style={{ marginTop: 0, marginBottom: 10 }}>
                  npm membership isn&rsquo;t public. Run these locally (you must
                  be authenticated), then paste the JSON output below:
                </p>
                ...
                <textarea
                  ...
                  placeholder={'Paste output of `npm org ls <org> --json` here.\nMultiple orgs can be pasted one after another.'}
                />
                <p className="desc">
                  Parsed {members.length} member
                  {members.length === 1 ? '' : 's'}. Matching is
        ...
        <section>
          ...
          <p className="note">
            ...
            — Ghostty&rsquo;s VT100 parser compiled to WebAssembly. Progress
            mirrors the original shell scripts&rsquo; stderr. Failed fetches are
            retried with backoff and counted, so a rate-limited package never
            silently looks &ldquo;clean.&rdquo;
          </p>
        </section>
      </div>
      ...
      {hasReports && result && (
        <section className="results" id="reports">
          <div className="results__head">
            <h2>Audit results</h2>
            <span className="hint">
              switch between reports with the tabs below
            </span>
          </div>
      ...
      <section className="panel" style={{ marginTop: 36 }}>
        ...
        <div className="panel__body">
          <p className="desc" style={{ marginTop: 0 }}>
            Versions a specific npm user personally published (recorded as the
            version&rsquo;s <code>_npmUser</code>) within the window. The package
            universe is the user&rsquo;s own maintained packages, optionally
            unioned with the packages from the last audit run.
          </p>
          <div className="row">
            <div className="field">
      ...
      <footer className="footer">
        ...
        : staged publish &gt; trusted publisher (OIDC + provenance) &gt;
        provenance &gt; none. Discovery via fast-npm-meta (npm.antfu.dev); trust
        from per-version registry manifests; weekly downloads from
        api.npmjs.org. &ldquo;Recency&rdquo; is the latest dist-tag&rsquo;s
