...
export async function discoverInScope(
  ...
  const meta = await resolveMeta(pkgs, failures, (d, t) =>
    log(`[recent]   resolved ${d}/${t}`),
  )
  const scope = inScope(meta, config)
  const scopeLabel = config.all
    ? 'ALL org packages'
    : `last ${config.months} months`
...
export async function runRecent(
  ...
  log(
    `[recent] fetching per-version manifests for trust status (${inScopeMeta.length})...`,
  )

  let done = 0
  const rows: RecentRow[] = await mapLimit(
    inScopeMeta,
    config.jobs,
    async (m) => {
  ...
  log(
    `[recent] fetching weekly downloads (bulk unscoped + ${nScoped} scoped paced ~2/s)...`,
  )
...
export async function runManual(
  ...
  let done = 0
  const nested: ManualRow[][] = await mapLimit(
    packages,
    config.jobs,
    async (pkg) => {
...
export async function runExternal(
  ...
  const pkgs = await listOrgPackages(config.orgs, failures)
  log(
    `[external] scanning ${pkgs.length} packages for current maintainers...`,
  )
  let done = 0
  const nested: ExternalRow[][] = await mapLimit(
    pkgs,
    config.jobs,
    async (pkg) => {
      const doc = await npmGetJson<MaintainerDoc>(pkgUrl(pkg), failures)
      done++
      if (done % 25 === 0 || done === pkgs.length)
        log(`[external]   scanned ${done}/${pkgs.length}`)
