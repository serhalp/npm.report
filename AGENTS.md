...
Every edge-reachable alias must also have an exact `.ts` mapping in
`import_map.json`: Netlify's deploy-time Deno bundler does not resolve the
`package.json` wildcard mappings.
...
- Edge-reachable internal aliases, `valibot`, and `packumeta` are mapped in
  `import_map.json`. Keep internal mappings synchronized with their
  `package.json#imports` targets and pin third-party esm.sh builds.
